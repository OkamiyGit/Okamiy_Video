...
import com.example.wangqing.okamiy_video.utils.MD5;
 ...
 * Letv分类接口：获取该Site下面的所有频道/该频道下的专辑
 */
...
public class LetvApi extends BaseSiteApi {
    ...
    private static final int BITSTREAM_SUPER = 100;//标识超清
    private static final int BITSTREAM_NORMAL = 101;//标识标清
    ...
    private Long mTimeOffSet = Long.MAX_VALUE;//时间戳，默认最大值
    ...
    private void fetchServerTime() {
        ...
        OkHttpUtils.excute(SEVER_TIME_URL, new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Log.d(TAG, ">>      获取时间戳 onFailure !!        >>");
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                if (!response.isSuccessful()) {
                    Log.i(TAG, ">>        获取时间戳  onResponse failed!!     >>");
                    return;
                }
                ...
                try {
                    ...
                    Log.i(TAG, ">>        获取时间戳  onResponse successd :    >>"+mTimeOffSet);
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    if (albumJson.optJSONObject("body") != null) {
                        ...
                        Log.i(TAG, ">>      乐视详情页  onGetAlbumDetail 获取的数据    >>" + albumJsonBody.toString());
                    ...
                    if (resultJson.optJSONObject("body") != null) {
                        ...
                        if (jsonArray != null) {
                            if (jsonArray.length() > 0) {
                                Log.i(TAG, ">>          乐视详情页中间部分 onGetVideo   获取的数据          >>: " + jsonArray.toString());
    ...
    public void onGetVideoPlayUrl(final Video video, final OnGetVideoPlayUrlListener listener) {
        ...
        Log.i(TAG, ">>             乐视 获取所有播放地址   onGetVideoPlayUrl  :               >>"+url);
        OkHttpUtils.excute(url, new Callback() {
                ...
                try {
                    ...
                    if (infosJson != null) {
                        ...
                        if (normalInfoObject != null) {
                            String normalUrl = "";
                            if (!TextUtils.isEmpty(normalInfoObject.optString("mainUrl"))) {//主播放地址
                                normalUrl = normalInfoObject.optString("mainUrl");
                                normalUrl += VIDEO_REAL_LINK_APPENDIX;
                            } else if (!TextUtils.isEmpty(normalInfoObject.optString("backUrl1"))) {//备份放地址1
                                normalUrl = normalInfoObject.optString("backUrl1");
                                normalUrl += VIDEO_REAL_LINK_APPENDIX;
                            } else if (!TextUtils.isEmpty(normalInfoObject.optString("backUrl2"))) {//备份放地址2
                        ...
                        if (superfoObject != null) {
                            ...
                            if (!TextUtils.isEmpty(superfoObject.optString("mainUrl"))) {
                                ...
                                superUrl += VIDEO_REAL_LINK_APPENDIX;
                            } else if (!TextUtils.isEmpty(superfoObject.optString("backUrl2"))) {
        ...
        private void getRealUrl(final Video video, String normalUrl, final int type, final OnGetVideoPlayUrlListener listener) {
            OkHttpUtils.excute(normalUrl, new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    // Nothing
                    Log.i(TAG, ">>            乐视获取真是播放地址失败  onFailure:    !!           >>");
                }
        ...
        private String getCurrentServerTime() {
            if (mTimeOffSet != Long.MAX_VALUE) {//不相等说明已经请求过了，
